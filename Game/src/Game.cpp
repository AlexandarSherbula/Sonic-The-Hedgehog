...
bool Game::OnStart()
{
    currentAct = std::make_unique<Act>();
    currentAct->Load("GHZ_Act_1", "GHZ_Tiles");

    cameraController = std::make_unique<CameraController>();

    player = std::make_unique<Player>(Alexio::Vector2(212.0f, 492.0f), 8, 16, Direction::RIGHT, "assets/images/character/SonicSpriteSheet.png");
...
bool Game::OnFixedUpdate()
{
    ...
    cameraController->Update(player.get(), currentAct->screenSize);
