...
float4 PSMain(PS_INPUT input) : SV_TARGET
{
    ...
    float4 targetColors[2];
    
    targetColors[0] = float4(0.0, 240.0 / 255.0, 0.0 / 255.0, 1.0);
    targetColors[1] = float4(0.0, 138.0 / 255.0, 0.0 / 255.0, 1.0);
    ...
    for (int i = 0; i < 2; i++)
    {
        if (IsEqual(pixel, targetColors[i], epsilon))
        {
            pixel = newColor;
        }
    }
